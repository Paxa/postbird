...
#define ThrowException	Isolate::GetCurrent()->ThrowException
#define THROW(x, m) return uni::Return(ThrowException(x(String::NewFromUtf8(Isolate::GetCurrent(), m))), args)
#define Dispose Reset
#define Undefined() Undefined(Isolate::GetCurrent())
#define SetResourceConstraints(x) SetResourceConstraints(Isolate::GetCurrent(), x)
#define NewSymbol(x) NewFromUtf8(Isolate::GetCurrent(), x)
#ifdef DEBUG
...
namespace uni {
	...
	template <void (*F)(void*)>
	  void WeakCallbackShim2(const v8::WeakCallbackData<v8::Object, void>& data) {
	    v8::Isolate* isolate = data.GetIsolate();
	    v8::HandleScope scope(isolate);
	    void* d = data.GetParameter();
	    F(d);
	}
	...
	void MakeWeak(Isolate* isolate, Persistent<T>& handle, P* val) {
		...
		handle.SetWeak(val, WeakCallbackShim2<F>);
	}
	template <class T>
...
class Fiber {
	...
	private:
		...
		static uni::FunctionType New(const uni::Arguments& args) {
			...
			new Fiber(args.This(), fn, Isolate::GetCurrent()->GetCurrentContext());
		...
		static uni::FunctionType Run(const uni::Arguments& args) {
			...
			if (!that.started) {
				...
				that.started = true;
				Isolate::GetCurrent()->AdjustAmountOfExternalAllocatedMemory(that.this_fiber->size() * GC_ADJUST);
			} else {
		...
		void UnwindStack() {
			...
			Handle<Value> zombie_exception = Exception::Error(String::NewFromUtf8(Isolate::GetCurrent(), "This Fiber is a zombie"));
		...
		static void RunFiber(void** data) {
			...
			// this function won't be called.
			{
				...
				Script::Compile(String::NewFromUtf8(Isolate::GetCurrent(), "void 0;"));
				...
				// otherwise.
				Isolate::GetCurrent()->AdjustAmountOfExternalAllocatedMemory(-(int)(that.this_fiber->size() * GC_ADJUST));
		...
		static uni::FunctionType GetStarted(Local<String> property, const uni::GetterCallbackInfo& info) {
			...
			return uni::Return(Boolean::New(Isolate::GetCurrent(), that.started), info);
		}
		...
		static uni::FunctionType GetPoolSize(Local<String> property, const uni::GetterCallbackInfo& info) {
			return uni::Return(Number::New(Isolate::GetCurrent(), Coroutine::pool_size), info);
		}
		...
		static uni::FunctionType GetFibersCreated(Local<String> property, const uni::GetterCallbackInfo& info) {
			return uni::Return(Number::New(Isolate::GetCurrent(), Coroutine::coroutines_created()), info);
		}
	...
	public:
		...
		static void Init(Handle<Object> target) {
			...
			// Fiber constructor
			Handle<FunctionTemplate> tmpl = FunctionTemplate::New(Isolate::GetCurrent(), New);
			...
			Handle<Signature> sig = Signature::New(Isolate::GetCurrent(), tmpl);
			tmpl->InstanceTemplate()->SetInternalFieldCount(1);

			// Fiber.prototype
			Handle<ObjectTemplate> proto = tmpl->PrototypeTemplate();
			proto->Set(String::NewSymbol("reset"),
				FunctionTemplate::New(Isolate::GetCurrent(), Reset, Handle<Value>(), sig));
			proto->Set(String::NewSymbol("run"),
				FunctionTemplate::New(Isolate::GetCurrent(), Run, Handle<Value>(), sig));
			proto->Set(String::NewSymbol("throwInto"),
				FunctionTemplate::New(Isolate::GetCurrent(), ThrowInto, Handle<Value>(), sig));
			proto->SetAccessor(String::NewSymbol("started"), GetStarted);

			// Global yield() function
			Handle<Function> yield = FunctionTemplate::New(Isolate::GetCurrent(), Yield_)->GetFunction();
...
extern "C" void init(Handle<Object> target) {
	if (did_init || !target->Get(String::NewFromUtf8(Isolate::GetCurrent(), "Fiber"))->IsUndefined()) {
