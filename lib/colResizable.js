...
(function ($) {
    ...
    var init = function (tb, options) {
        ...
        /*
        var th = table.find("thead > tr > th, thead > tr > td")
        table.ln = th.length;
        th.each(function() { table.c.push($(this)); });
    ...
    function supposedWidth(column) {
      var i = parseInt;
      var width = Math.max(i(column.css('width')), i(column.css('min-width')));
      return  width +
              i(column.css('padding-left')) +
              i(column.css('padding-right')) +
              i(column.css('border-left-width')) +
              i(column.css('border-right-width'));
    }

    var syncTable = function syncTable(table) {
      var totalWidth = 0;
      for (var i = 0; i < table.ln; i++) {
        totalWidth += supposedWidth(table.c[i]);
      }
